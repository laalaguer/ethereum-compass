...
树的结构中常见的名词解释如下。
  ...
  - 节点（node）：用圆圈表示，节点包含键、值、与指向后继节点的路径。
  - 树根（root）：树的起始节点，一棵树有且仅有一个树根。
  - 树叶（leaf）：树的最下层末梢节点，再无后继节点。
  - 内部节点（internal node）: 树中间的节点，承上启下，在上层与后继都有节点。
  - 路径（Path）：连结两个节点之间的直线，可以是双向箭头，也可以是单向箭头。
